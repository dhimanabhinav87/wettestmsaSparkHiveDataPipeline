...
object WettestMsaYearMonth {
  ...
  def loadStationNames(path:String) : Map[Int, String] = {

    // Create a Map of Ints to Strings, and populate it from u.item.
    var stationNames:Map[Int, String] = Map()

    //val lines = scala.io.Source.fromFile("/home/cloudera/FinalProject/QCLCD201505/201505station.txt").getLines()
  ...
  def main(args: Array[String]) = {
    ...
    //these variables are defined in config file
    val precip_file_path= p_nn_server+ p.apply("LANDING_PRECIPITATION")
    val city_state_msa_file_path= p_nn_server+ p.apply("LANDING_CITY_STATE_MSA")
    val pop_msa_file_path= p_nn_server+ p.apply("LANDING_POP_MSA")
    //since station file path is a local file path, passing this as argument to spark-submit
    val station_file_path= args(1)
    val output_file_path=p_nn_server+ p.apply("OUTPUT_PATH")


    //load precipitation file in Rdd from HDFS
    val precipLines = sc.textFile(precip_file_path)
    ...
    val stationNameDict = sc.broadcast(loadStationNames(station_file_path))
    ...
    //load citystate->msa
    val cityStateMsaLines = sc.textFile(city_state_msa_file_path)
    ...
    val msaPopLines= sc.textFile(pop_msa_file_path)
    ...
    df_wettestmsa_repartitioned.write.format("com.databricks.spark.csv").option("delimiter", "\t").mode("overwrite").save(output_file_path)
  }
